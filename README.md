...
Androidの開発者向けオプション「USBオーディオルーティングを無効化」を切り替え、現在のメディア出力先や音量を確認するための小さなアプリです。

USBプラグと音波を組み合わせた専用のアダプティブ・ランチャーアイコンを同梱しています。

現在のメディア出力先と音量メーターを表示するホーム画面ウィジェットも同梱しています。ウィジェット内の「更新」で即時取得でき、音量操作時にも連動更新されます。Samsung DeXのホーム画面へのウィジェット配置はOne UI 8（Android 16）以降で利用できます。
